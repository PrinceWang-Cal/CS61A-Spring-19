...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if not self.forward and not self.scared:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_location != hive:
            bee_appearance = random_or_none(current_location.bees)
            if current_location.bees and bee_range >= self.min_range and bee_range <= self.max_range:
                return bee_appearance
...
class FireAnt(Ant):
    ...
    """
    write a conditional statement here so that when fireant dies reduce_armor get executed
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor == 0:
            return [Insect.reduce_armor(bee, self.damage) for bee in location.bees[:]]
...
class WallAnt(Ant):
    "A WallAnt with large armor value"
    name = "Wall"
    food_cost = 4
    Implemented = True
...
# END Problem 13
    ...
    0) Attacks the nearest bee and doubles the damage of all the ants
    ...
    is_queenant = True
    implemented = True
    queen_exist = False# Change to True to view in the GUI
    # END Problem 13

    def __init__(self, armor=1):
        # BEGIN Problem 13
        self.armor = armor
        if not QueenAnt.queen_exist:
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            while location != None:
                if location.ant:
                    if location.ant.buffed == False:
                        location.ant.damage = location.ant.damage*2
                        location.ant.buffed = True
                    if isinstance(location.ant, BodyguardAnt) or isinstance(location.ant, TankAnt):
                        if location.ant.contained_ant and location.ant.contained_ant.buffed == False:
                            location.ant.contained_ant.damage = location.ant.contained_ant.damage*2
                            location.ant.contained_ant.buffed = True
...
def make_slow(action, bee):
    ...
    """
    # BEGIN Problem EC
    def slow_action(colony, duration):
        if duration == 0:
            action(colony)
        else:
            if colony.time % 2 == 0:
...
def make_scare(action, bee):
    ...
    """
    # BEGIN Problem EC
    def scared_action(colony, duration):
        if duration == 0:
            bee.scared = True
            action(colony)
        else:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    effect_call = effect(bee.action, bee)
    def helper(colony):
        nonlocal duration
        if duration > 0:
            effect_call(colony,duration)
            duration -= 1
        elif duration == 0:
            effect_call(colony,duration)
    bee.action = helper
