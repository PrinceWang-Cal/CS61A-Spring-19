...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        rrest= rest.map(lambda a: scheme_eval(a, env))
        return scheme_apply(proc, rrest, env)
        # END PROBLEM 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            if (vals.second is nil) and (formals.second is not nil):
                raise SchemeError
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        bin = bindings.first
        check_form(bin, 2, 2)
        formals = Pair(bin.first, formals)
        check_formals(formals)
        arg = Pair(scheme_eval(bindings.first.second.first, env),arg)
        bindings = bindings.second
    return env.make_child_frame(formals, arg)
    # END PROBLEM 15
